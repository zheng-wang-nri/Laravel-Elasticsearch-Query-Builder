...
//				}
	...
	public function whereHas($column, $closure = null, $or = false, $boost = false) {
		...
		$builder = $this->nested_queries[$column_bak] ?? new LaravelElasticsearchQueryBuilder($this->model, Str::snake($column_bak));
		$closure($builder);
	...
	public function whereHasNull($column, $closure = null, $or = false) {
		if($closure === null) {
			 ...
			 * Prepend index name to column name
			 */
			$column_field = $this->getIndexName().'.'.Str::snake($column);
	...
	public function orderBy(string $column, $order = 'asc', $script = false) {
		 ...
		 * prepend index Name to column name
		 */
		$column_field = $this->getIndexName().'.'.Str::snake($column) . $sub_field;
